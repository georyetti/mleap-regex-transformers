...
/**
 * Spark transformer to execute a RegexExtractModel against a Spark dataframe. Extends [[Transformer]].
 *
 * @param uid Unique transformer name
 * @param model Model to execute, of class [[RegexExtractModel]].
 */
class RegexExtract(override val uid: String = Identifiable.randomUID("regex_extract"),
                   val model: RegexExtractModel)
  extends Transformer
    with HasInputCol
    with HasOutputCol {
  ...
  override def copy(extra: ParamMap): Transformer =
    copyValues(new RegexExtract(uid, model), extra)
